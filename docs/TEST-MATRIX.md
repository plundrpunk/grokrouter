...
| Per-Bot provider/model state isolation | Two-Bot state test plus merge-under-lock writes; stable Bot identity outranks changing turn request IDs | Beta.44 switched the first post-install Bot to Luna; a second genuinely new Bot started on installer-default Claude | Pass on beta.44 |
...
| Fresh-Bot forgiving model controls | Unit test proves exact, capitalized, whitespace, invalid-command, listed, and unlisted model inputs never reach inference | Beta.44 passed Doctor, bare Luna switch, provider status, normal inference, every required near-miss, and `/models <id>` in a genuinely new Bot after restore/reinstall | Pass on beta.44 |
| Single delivery after visible assistant response | Unit and concurrency tests claim one user turn before provider inference, release failed claims for retry, and suppress completed host replays | Beta.44 returned one settled `FRESH_BOT_TEXT_OK` reply and one normal model-identity reply; both remained single after an additional ten-second observation | Pass on beta.44 |
| Routed model knows provider/model controls | OpenRouter request and Codex prompt contract tests | Beta.44 Luna identified `OpenRouter` and `openai/gpt-5.6-luna` on the turn after the switch | Pass on beta.44 |
| Native slash discovery | Payload/install tests verify six user-invocable skill descriptors, ownership-safe links, conflict reporting, and cleanup; runtime tests prove `/doctor` and group-addressed controls bypass inference | Beta.44's native menu offered `doctor`; the final reinstall reconciled six unique GrokRouter commands across 13 Bots and channels | Pass on beta.44 |
